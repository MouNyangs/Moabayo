...
public class AccountApplyServiceImpl implements AccountApplyService {
	  ...
	  public AccountApplyResponse apply(Long userId, AccountApplyRequest req) {
	    ...
	    Long uaId = nextUserAccountId(); // seq_user_account_id.NEXTVAL 호출용(네 Mapper에 맞춰 구현)
	  ...
	  private Long nextUserAccountId(){
	    // AccountMapper에 SELECT seq_user_account_id.NEXTVAL FROM dual 쿼리 만들어 두고 호출 권장
	    // return accountMapper.nextUserAccountId();
	    // 임시 더미(반드시 Mapper호출로 교체)
