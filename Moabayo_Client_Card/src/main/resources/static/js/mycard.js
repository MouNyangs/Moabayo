document.addEventListener('DOMContentLoaded', () => {
  ...
  const updateLimitBar = (limitText)=>{
    ...
    if (pct >= 90) bar.classList.add('is-danger'); else if (pct >= 70) bar.classList.add('is-warn');
    bar.style.width = pct + '%';
  ...
  function renderStatus(){
    ...
    if (els.stSel) els.stSel.innerHTML = `<span class="dot"></span>${selected ? ' 선택됨: ' + (selected.dataset.title || '') : ' 선택된 카드 없음'}`;
    els.stFav?.classList.toggle('fav', favored);
    if (els.stFav) els.stFav.innerHTML = `<span class="dot"></span>즐겨찾기: ${favored ? '켜짐' : '꺼짐'}`;
    if (els.stNext) els.stNext.innerHTML = `<span class="dot"></span>다음 납부일: ${selected?.dataset.next || '-'}`;
  }
  ...
  function clearActive(){ document.querySelectorAll('.card.is-active').forEach(el=>el.classList.remove('is-active')); }
  ...
  function onSelect(card){ selected = card; clearActive(); card.classList.add('is-active'); fillDetail(card); renderStatus(); }

  // 리스트 인터랙션
  els.list.addEventListener('click', e=>{ const card=e.target.closest('.card'); if(card) onSelect(card); });
  els.list.addEventListener('keydown', e=>{
    if((e.key==='Enter'||e.key===' ') && e.target.classList.contains('card')){ e.preventDefault(); onSelect(e.target); }
  });

  // 액션
  els.btnFav?.addEventListener('click', ()=>{ if(!selected) return alert('카드를 먼저 선택하세요.'); favored=!favored; renderStatus(); });
  els.btnShare?.addEventListener('click', async ()=>{
    ...
    try{ if(navigator.share) await navigator.share({ title:'모으냥즈 내 카드', text, url: location.href });
         else { await navigator.clipboard.writeText(text); alert('카드 상태가 복사되었습니다.'); } }catch(_){}
  });
  ...
  // 초기 상태
  renderStatus();
  const first = els.list.querySelector('.card'); if(first) onSelect(first);
});

/* =============================
   Fireflies (ultra lite)
   ============================= */
(function(){
  ...
  function spawn(i=0){
    ...
    return {
      x:rnd(0,W), y:rnd(0,H),
      vx:rnd(-CFG.SPEED,CFG.SPEED), vy:rnd(-CFG.SPEED,CFG.SPEED),
      r:rnd(CFG.R_MIN,CFG.R_MAX),
  ...
  function draw(ts){
    ...
    if(!prev) prev=ts;
    const dt=ts-prev; acc+=dt;
    if(acc < frame){ rafId=requestAnimationFrame(draw); return; }
    prev=ts; acc=0;
  ...
  // 자동 품질 하향(프레임이 자주 밀리면)
  let slowCount=0;
  setInterval(()=>{
    if(!running) return;
    // 최근 1초 평균 프레임 체크 (rough)
    if(acc > 20){ slowCount++; } else slowCount=0;
    if(slowCount>=3){ CFG.COUNT = Math.max(12, Math.floor(CFG.COUNT*0.8)); CFG.FPS = Math.max(24, CFG.FPS-2); slowCount=0; }
  }, 1000);

  // visibility/focus 최적화
