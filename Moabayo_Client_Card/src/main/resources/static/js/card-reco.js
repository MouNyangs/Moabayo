// === card-reco.js (Python fetch 연동 버전) =====================================
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // === Python / Spring Boot에서 데이터 fetch ===
    ...
    .then(data => {
      ...
      if(fitCtx){
        new Chart(fitCtx, {
          ...
          data:{
            labels:['적합도','나머지'],
            datasets:[{
              data:[data.fit, 100-data.fit],
              cutout:'76%',
              backgroundColor:[COL.brand,'rgba(128,128,128,.18)'],
              borderWidth:0
            }]
          },
          options:{ plugins:{legend:{display:false}, tooltip:{enabled:false}}, responsive:true, maintainAspectRatio:false }
        });
        if(fitVal) fitVal.textContent = data.fit;
      }
      ...
      if(timeBar){
        new Chart(timeBar, {
          ...
          data:{
            labels:['아침','점심','저녁','심야'],
            datasets:[{ data:data.time, backgroundColor:COL.brand, borderRadius:6 }]
          },
      ...
      if(weekLine){
        new Chart(weekLine, {
          ...
          data:{
            ...
            datasets:[{ data:data.week, borderColor:COL.brand, tension:.35, fill:false, pointRadius:0, borderWidth:2 }]
          },
          options: miniOpts({ showXTicks:true })
        });
      }
